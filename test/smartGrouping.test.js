...
test('prefers existing group titles before url rules', () => {
  const draft = createSmartGroupDraft({
    ...
    groups: [{ id: 21, title: '服务商&商家工作台' }, { id: 13, title: 'ai' }],
    rulesText: 'ai/partner.jinritemai.com'
  });
  ...
  assert.deepEqual(draft.suggestions[0], {
    ...
    targetGroupId: 21,
    newGroupTitle: '',
    reason: '标题匹配「服务商&商家工作台」'
  });
});

test('uses rules to suggest existing or new groups', () => {
  const draft = createSmartGroupDraft({
    tabs: [
    ...
    groups: [{ id: 13, title: 'ai' }],
    rulesText: '邮箱/mail.example.com'
  });
  ...
  assert.equal(draft.suggestions[1].newGroupTitle, '邮箱');
});
...
test('only suggests existing groups in the same window', () => {
  ...
  assert.equal(draft.suggestions[0].newGroupTitle, 'ai');
});
